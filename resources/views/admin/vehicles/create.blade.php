...
@section('content')
    <div class="mx-auto max-w-3xl rounded-3xl border border-slate-800/70 bg-slate-900/70 p-8 shadow-xl shadow-slate-950/20">
        <div class="mb-8">
            <h1 class="text-2xl font-semibold text-white">Tambah Kendaraan</h1>
            <p class="mt-2 text-sm text-slate-500">Daftarkan kendaraan yang digunakan untuk operasi keluar-masuk troli.</p>
        </div>

        <form method="POST" action="{{ route('vehicles.store') }}" class="space-y-6">
            @csrf
            @include('admin.vehicles.partials.form', ['vehicle' => null])
