...
@section('content')
    <div class="rounded-3xl border border-slate-800 bg-slate-900/70 shadow-xl shadow-slate-950/20">
        <div class="flex flex-col gap-4 border-b border-slate-800 px-6 py-6 md:flex-row md:items-center md:justify-between">
            <div>
                <h1 class="text-xl font-semibold text-white">Persetujuan Akun Mobile</h1>
                <p class="text-sm text-slate-500">Kelola workflow persetujuan pengguna aplikasi mobile.</p>
            </div>
            <div class="flex items-center gap-2">
                <a href="{{ route('admin.approvals.index', ['status' => 'pending']) }}" class="rounded-xl px-4 py-2 text-xs font-semibold uppercase tracking-wide {{ $status === 'pending' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/30' : 'border border-slate-700 bg-slate-800/80 text-slate-300 hover:bg-slate-800' }}">
                    Pending
                </a>
                <a href="{{ route('admin.approvals.index', ['status' => 'approved']) }}" class="rounded-xl px-4 py-2 text-xs font-semibold uppercase tracking-wide {{ $status === 'approved' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-600/30' : 'border border-slate-700 bg-slate-800/80 text-slate-300 hover:bg-slate-800' }}">
                    Disetujui
                </a>
                <a href="{{ route('admin.approvals.index', ['status' => 'rejected']) }}" class="rounded-xl px-4 py-2 text-xs font-semibold uppercase tracking-wide {{ $status === 'rejected' ? 'bg-rose-600 text-white shadow-lg shadow-rose-600/30' : 'border border-slate-700 bg-slate-800/80 text-slate-300 hover:bg-slate-800' }}">
                    Ditolak
                </a>
            </div>
        </div>
